...
(async()=>{
  ...
  ok(lastCol.every(t=>t.trim()==='—'),'Last collection is empty for every row');
  ...
  await p.selectOption('#uStatus','supported'); await p.waitForTimeout(250);
  const sup=await p.locator('#uBody tr td:nth-child(5)').allTextContents();
  ok(sup.length>0 && sup.every(t=>/Supported/.test(t)),'status filter returns only Supported');
  ...
  ok(/Detector available/.test(cards)&&/Not implemented/.test(cards),'connector cards reflect the real registry');
